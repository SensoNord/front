...
import {
    fetchFolderById,
    fetchFolderByParent, folderByParentPayload,
...
export default function DisplayFiles(props: Props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await dispatch(fetchFolderByParent({parentId: actualFolder.id, connectedUserId: connectedUser.id} as folderByParentPayload));
                            ...
                            folderListToDisplay.map((folder) => {
                                return (
                                    <div key={folder.id} onClick={() => handleClickFolder(folder)}>
                                        {folder.name}{folderListToDisplay[folderListToDisplay.length - 1] === folder ? '' : ' >'}&nbsp;
                                    </div>
                                );
                            })
                        }
                                            ...
                                            return (
                                                <>
                                                    <div
                                                        ...
                                                        className={`cursor-pointer flex w-full ${compactMode ? '' : 'my-2'}`}
                                                    >
                                                ...
                                                <>
                                                    <div key={file.id} className={"grid grid-cols-12 my-2 w-full"}>
                                                        <div
                                                            ...
                                                            className={`cursor-pointer flex col-span-6 ${compactMode ? '' : 'my-2'}`}
                                                        >
                                                            {
                                                        ...
                                                        <div className={compactMode ? 'col-span-6' : "col-span-3"}>
                                                            {/*{(new Date(file.modified_on)).toLocaleString('fr-FR')}*/}
                                                            {
                                                                (file.modified_on !== null && file.modified_by !== null) ? (
                                                                    <NameAndDate date_created={new Date(file.modified_on)} user_created={{first_name: file.modified_by.first_name, last_name: file.modified_by.last_name} as UserType}></NameAndDate>
                                                                ) : (
                                                                    <NameAndDate date_created={new Date(file.uploaded_on)} user_created={{first_name: file.uploaded_by.first_name, last_name: file.uploaded_by.last_name} as UserType}></NameAndDate>
                                                                )
                                                            }
                                                        </div>
                                                        {!compactMode && (
